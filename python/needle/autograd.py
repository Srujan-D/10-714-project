...
class SparseTensor(Value):
        ...
        **kwargs
        ...
        if isinstance(data, SparseTensor):
            ...
            if device == data.device and dtype == data.dtype:
                data, indices, shape = data.realize_cached_data()
            else:
                # fall back, copy through numpy conversion
                data = SparseTensor._array_from_numpy(
                    data.numpy(), device=device, dtype=dtype
                )
        else:
            device = device if device else default_device()
            data = SparseTensor._array_from_numpy(data, device=device, dtype=dtype)
    ...
    def __add__(self, other):
        if isinstance(other, SparseTensor):
            return needle.ops.SparseEWiseAdd()(self, other)
        elif isinstance(other, Tensor):
            return needle.ops.SparseEWiseAddTensor()(self, other)
        else:
            return needle.ops.SparseAddScalar(other)(self)

    def __mul__(self, other):
        if isinstance(other, SparseTensor):
            return needle.ops.SparseEWiseMul()(self, other)
        elif isinstance(other, Tensor):
            return needle.ops.SparseEWiseMulTensor()(self, other)
        else:
            return needle.ops.SparseMulScalar(other)(self)

    # check if we actually need power kernel
    # def __pow__(self, other):
    #     if isinstance(other, SparseTensor):
    #         return needle.ops.SparseEWisePow()(self, other)
    #     else:
    #         return needle.ops.SparsePowerScalar(other)(self)
    
    def __sub__(self, other):
        if isinstance(other, SparseTensor):
            return needle.ops.SparseEWiseAdd()(self, needle.ops.SparseNegate()(other))
        elif isinstance(other, Tensor):
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        for k, input_node_k in enumerate(node.inputs):
            if input_node_k not in node_to_output_grads_list:
                node_to_output_grads_list[input_node_k] = []
            node_to_output_grads_list[input_node_k].append(input_grads[k])
    ### END YOUR SOLUTION

...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        # Check if the matrix is compact
        if not self.is_compact():
            raise ValueError("Matrix is not compact")

        # Check if the product of current shape is equal to the product of the new shape
        current_shape_product = 1
        for dim in self.shape:
            current_shape_product *= dim

        new_shape_product = 1
        for dim in new_shape:
            new_shape_product *= dim

        if current_shape_product != new_shape_product:
            raise ValueError(
                "Product of current shape is not equal to the product of the new shape"
            )

        # Create a new NDArray with the new shape and the same memory
        # new_ndarray = NDArray.make(new_shape, device=self.device, handle=self._handle, offset=self._offset)
    ...
    def broadcast_to(self, new_shape):
        ...
        # Convert self shape to a list for easier manipulation
        original_shape = list(self._shape)
        original_strides = list(self._strides)

        # Prepend 1s to original_shape to match the length of new_shape if needed
        while len(original_shape) < len(new_shape):
            original_shape.insert(0, 1)
            original_strides.insert(0, 0)
        ...
        for orig_dim, new_dim in zip(original_shape, new_shape):
            if orig_dim != 1 and orig_dim != new_dim:
                raise AssertionError(
                    f"Shape mismatch: cannot broadcast shape {self._shape} to {new_shape}"
                )

        # Compute new strides for the broadcasted shape
        new_strides = []
        for orig_dim, stride, new_dim in zip(
            original_shape, original_strides, new_shape
        ):
            if orig_dim == new_dim:
                new_strides.append(stride)
            elif orig_dim == 1:
                new_strides.append(0)  # Broadcasting dimension

        # Return a new NDArray with the new shape and broadcasted strides
        return NDArray.make(
            shape=new_shape,
    ...
    #         original_shape.insert(i, 1)
    #     if original_shape[i] != 1 and original_shape[i] != new_shape[i]:
    #         raise AssertionError("shape mismatch")

    # # Calculate the new strides.
    # new_strides = [0] * len(new_shape)  # Initialize with zeros
    # for i in range(len(new_shape)):
    #     if i < len(self._shape) and (self._shape[i] == new_shape[i]):
    #         new_strides[i] = self._strides[i]
    #     elif i < len(self._shape) and (self._shape[i] == 1):
    #         new_strides[i] = 0
    #     else:
    #         new_strides[i] = new_strides[i - 1] * new_shape[i - 1]

    # # Return a new NDArray with the new shape and strides.
    # return NDArray.make(
    #     shape=new_shape,
    #     strides=tuple(new_strides),
    #     device=self._device,
    ...
    #         if dim != 1 and dim != new_shape[i]:
    #             raise AssertionError("shape mismatch")

    #     # Calculate the new strides.
    #     new_strides = [0] * len(new_shape)  # Initialize with zeros
    #     for i, dim in enumerate(new_shape):
    #         if dim == self._shape[i]:  # If dimension size is same, use old stride
    #             new_strides[i] = self._strides[i] if i < len(self._strides) else 0
    #         elif self._shape[i] == 1:  # If old dimension was 1, stride is 0
    #             new_strides[i] = 0
    #         else:
    #             raise AssertionError(
    #                 "shape mismatch"
    #             )  # This should be caught in the previous check
    ...
    def __getitem__(self, idxs):
        ...
        new_stride = tuple([self._strides[i] * idx.step for i, idx in enumerate(idxs)])

        # print('new_stride', self._strides, new_stride)
    ...
    def flip(self, axes):
        ...
        for ax in axes:
            new_stride[ax] = -new_stride[ax]
        new_offset = 0
        for i in axes:
            new_offset += (self._shape[i] - 1) * self._strides[i]
        return NDArray.make(
            shape=self._shape,
    ...
    def pad(self, axes):
        ...
        new_array = NDArray.make(tuple(new_shape), device=self.device)
        new_array.fill(0)
        slices = [slice(ax[0], ax[0]+self.shape[i], 1) for i, ax in enumerate(axes)]
        # print("new_array created", slices)
...
def copy(a):
    return NDArray(a.numpy(), device=a.device)


def stack(arrays, axis=0):
    return NDArray(np.stack([a.numpy() for a in arrays], axis=axis))


def split(a, indices_or_sections, axis=0):
    return np.split(a.numpy(), indices_or_sections, axis=axis)


def swapaxes(a, axis1, axis2):
    return a.permute(
        [axis2 if i == axis1 else axis1 if i == axis2 else i for i in range(a.ndim)]
    )

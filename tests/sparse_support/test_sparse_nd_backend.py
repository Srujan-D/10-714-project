...
def test_sparse_add():
    a = nd.sparse_ndarray.SparseNDArray(np.array([[2, 0, 0], [0, 0, 5]], dtype=np.float32), device=ndl.cpu())
    b = nd.sparse_ndarray.SparseNDArray(np.array([[1, 0, 0], [0, 2, 0]], dtype=np.float32), device=ndl.cpu())
    # breakpoint()
...
def test_sparse_mul():
    a = nd.sparse_ndarray.SparseNDArray(np.array([[1, 0, 0], [0, 0, 1]], dtype=np.float32), device=ndl.cpu())
    b = nd.sparse_ndarray.SparseNDArray(np.array([[0, 0, 1], [0, 1, 0]], dtype=np.float32), device=ndl.cpu())
...
def test_sparse_sparse_matmul():
    def run_sparse_test():
        ...
        mat_1_sparse = nd.sparse_ndarray.SparseNDArray(mat_1, device=ndl.cpu())
        # mat_2_sparse = nd.sparse_ndarray.SparseNDArray(mat_2, device=ndl.cpu())

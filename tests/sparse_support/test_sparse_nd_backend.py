...
def test_sparse_sparse_matmul():
    def run_sparse_test():
        a = np.random.randint(low=10, high=50)
        b = np.random.randint(low=10, high=50)
        c_ = np.random.randint(low=10, high=50)
    ...
    avg_time = timeit.timeit(run_sparse_test, number=1) / 1
...
#     avg_time = timeit.timeit(run_dense_test, number=20) / 20

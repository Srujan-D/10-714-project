...
def test_sparse_add():
    a = nd.sparse_ndarray.SparseNDArray(np.array([[2, 0, 0], [0, 0, 5]], dtype=np.float32), device=ndl.cuda())
    b = nd.ndarray.array(np.array([[1, 0, 0], [0, 2, 0]], dtype=np.float32), device=ndl.cuda())
    # breakpoint()
...
def test_sparse_mul():
    a = nd.sparse_ndarray.SparseNDArray(np.array([[1, 0, 0], [0, 0, 1]], dtype=np.float32), device=ndl.cuda())
    b = nd.ndarray.array(np.array([[0, 0, 1], [0, 1, 0]], dtype=np.float32), device=ndl.cuda())
...
def test_sparse_sparse_matmul():
    def run_sparse_test():
        ...
        mat_1_sparse = nd.sparse_ndarray.SparseNDArray(mat_1, device=ndl.cuda())
        # mat_2_sparse = nd.sparse_ndarray.SparseNDArray(mat_2, device=ndl.cuda())
        mat_2_dense = nd.ndarray.array(mat_2, device=ndl.cuda())
...
def test_dense_matmul_for_sparse_matrices():
    def run_dense_test():
        ...
        mat_1_dense = nd.ndarray.array(mat_1, device=ndl.cuda())
        mat_2_dense = nd.ndarray.array(mat_2, device=ndl.cuda())
